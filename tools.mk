...
${LOCAL_PLT}: compile
	@if [ -d $(DEP_DIR) ]; then \
		if [ -f $(LOCAL_PLT) ]; then \
			dialyzer --check_plt --plt $(LOCAL_PLT) $(DEP_DIR)/*/ebin  && \
			dialyzer --add_to_plt --plt $(LOCAL_PLT) --output_plt $(LOCAL_PLT) $(DEP_DIR)/*/ebin ; test $$? -ne 1; \
		else \
			dialyzer --build_plt --output_plt $(LOCAL_PLT) $(DEP_DIR)/*/ebin ; test $$? -ne 1; \
		fi \
	fi
